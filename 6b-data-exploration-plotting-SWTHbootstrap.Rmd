---
title: "Lesson 6b. Data Exploration and Plotting in R - Swainson's Thrush Data - Adding Bootstrap"
author: "L Leston"
date: "2023-07-27"
...
Let's create some time variables for each recording. 
...
GAM3 <- gamm4(SWTH ~latitude.F+ s(JDAY, k=3, bs = "cs", by=latitude.F) + s(TIME, k=5, bs = "cc", by=latitude.F), random=~(1|location), data=songs_per_recording, family=negbin(theta=thetaval, link="log"))
...
table(songs_per_recording$location)#4 sites with 197 - 418 recordings per site
...
d1<-fSampleNint(songs_per_recording)
d2<-fSampleNint(songs_per_recording)
d3<-fSampleNint(songs_per_recording)

bs <- function(data, indices){#
  d<-fSampleNint(data)#, PKEY, 1
  #return(d$PKEY)
  thetaval<-var(d$SWTH)/mean(d$SWTH)
  try(GAM3 <- gamm4(SWTH ~latitude.F+ s(JDAY, bs = "cs", k=3, by=latitude.F) + s(TIME, k=5, bs = "cc", by=latitude.F), random=~(1|location), data=d, family=negbin(theta=thetaval, link="log")))
  try(coef.gamm<-coef(GAM3$gam))
  try(return(coef.gamm))
}
#specifying k=3 for a quadratic function (single peak) and k=5-6 for two peaks that don't necessary fall at extreme values of a predictor.
library(boot)
a<-Sys.time()
songresults <- boot(data=songs_per_recording,
...
save(songresults, CoefB, CoefBCI50, CoefBCI90, file="output/boot100_SWTH_Aug4_3k_Julian_5k_Time.RData")

```

Plot bootstrapped model results, producing a matrix plot of song activity versus time of day/night for each combination of latitude and Julian date. Note that the code chunk below has been set to *eval=FALSE*, so that it isn't executed when the Markdown document is knitted.

```{r bs-predictions, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
load("output/boot100_SWTH_Aug4_3k_Julian_5k_Time.RData")
songs_per_recording$date<-substr(songs_per_recording$recording_date_time,1,10)
dates<-data.frame(unique(songs_per_recording[,c("date","JulianDay","JDAY")]))
date_times<-data.frame(unique(songs_per_recording[,c("date","JulianDay","JDAY","timeNumeric","TIME")]))
...
for (i in 1:nrow(dates)){
  ...
  Latitude<-songs.tsss.jd$latitude.F
  timeNumeric<-songs.tsss.jd$timeNumeric
  str(songs.tsss.jd)
  names(songs.tsss.jd)
  
  songs.tsss.jdmm <- model.matrix(GAM3$gam, songs.tsss.jd)#meantemp.s+moon.s+
  ...
  songs.tsss.jd.bci$Latitude.F<-as.factor(songs.tsss.jd.bci$latitude.F)
  songs.tsss.jd.bci$date<-Day

  latlevels<-c("N","S")
  try(for (j in 1:length(latlevels)){
    preds.sitelevel<-songs.tsss.jd.bci[songs.tsss.jd.bci$Latitude.F==latlevels[j],]
    preds.025.975<-preds.sitelevel[,7:102]
    for (k in 1:ncol(preds.025.975)){
      preds.025.975[,k]<-ifelse(preds.025.975[,k]>20,20,preds.025.975[,k])#pick a threshold value that is reasonable for a 1-minute recording, e.g. a song every 3 seconds
    }
    tiff(paste0("output/matrix plots/SWTH 3k Julian 5k Time/",ifelse(j==1,"N_","S_"),"SWTHsongrateXToD-",Day,".tiff"), units="in", width=6, height=4, res=300)
    matplot(preds.sitelevel$timeNumeric, 
            preds.025.975, 
            type="l", cex.axis=1.5, cex.lab=1.5,
            col="red",#cividis(100),
            xlab=paste0("Time of day (24h) ", Day),
            xlim=c(0,23.99), ylim=c(0,20),
            ylab="SWTH song rate",
            main=paste0(ifelse(j==1,"North ","South "), "Alberta"))   
    dev.off()
  })
}
